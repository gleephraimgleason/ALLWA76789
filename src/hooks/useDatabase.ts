      ...
      if (!existingBalance) {
        ...
        if (insertError) {
          ...
          try {
            ...
            console.log("✅ Created minimal balance as fallback");
          } catch (fallbackError) {
            console.error(
              "💥 Failed to create even minimal balance:",
              fallbackError,
            );
          }
        } else {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      if (balanceRes.status === "fulfilled" && balanceRes.value.data) {
        ...
        setBalance({ dzd: 0, eur: 0, usd: 0, gbp: 0, investment_balance: 0 });
      }
      ...
      if (cardsRes.status === "fulfilled" && cardsRes.value.data) {
        ...
        console.warn("⚠️ Failed to load cards, using empty array");
        setCards([]);
      }
      ...
      if (
        ...
        console.warn("⚠️ Failed to load notifications, using empty array");
        setNotifications([]);
      }
      ...
      if (
        ...
        console.warn("⚠️ Failed to load investment balance, using 0");
